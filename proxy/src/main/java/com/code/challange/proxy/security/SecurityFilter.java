...
import com.code.challange.proxy.service.ProxyServiceImpl;
...
public class SecurityFilter extends GenericFilterBean {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            try {
                SecurityToken securityToken = getSecurityService(request).hasAccess(token, "/private");
                String username = securityToken.getUserName();
                List<GrantedAuthority> authorities = new ArrayList<>();
                if (securityToken.getRoles() != null) {
                    securityToken.getRoles().forEach(s -> authorities.add(new SimpleGrantedAuthority(s)));
                }
